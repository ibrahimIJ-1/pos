  ...
  const [
    ...
    prisma.$queryRaw<{ count: number }[]>`
            ...
            WHERE s.branch_id    = ${userBranchId}
              ...
              AND (si.quantity - si.refunded_quantity) > 0
        `,
...
`,
    ...
    prisma.$queryRaw<{ id: string; value: number }[]>`
            SELECT p.category as id,
                   SUM((si.quantity - si.refunded_quantity) * si.unit_price) as value
            FROM sale_items si
                JOIN products p
  ...
  const saleSum =
    totalRevenueResult.find((r) => r.type === TransactionType.SALE)?._sum
      .amount || 0;
  const refundSum =
    totalRevenueResult.find((r) => r.type === TransactionType.REFUND)?._sum
      .amount || 0;
